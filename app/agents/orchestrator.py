...
class AgentOrchestrator:
    ...
    async def _handle_chat_route(self, state: AgentState) -> str:
        system_prompt = (
            ...
            "форма. Отвечай по-русски. На короткие реплики вроде 'неплохо', "
            "'спасибо', 'ау', 'куку' отвечай естественно и коротко. Не пиши "
            "канцелярские фразы вроде 'сообщение принято', 'обратитесь в "
            "соответствующий отдел' или 'я не располагаю информацией', если "
            "можно ответить нормально. Если не знаешь чего-то внутреннего по "
            "компании, мягко попроси уточнить контекст. Если пользователь "
            "просит создать задачу, напоминание, изменить данные, файл или "
            "договор, не притворяйся что сделал это в чате: такие действия "
            "выполняются только через безопасный tool-flow."
        )
        try:
